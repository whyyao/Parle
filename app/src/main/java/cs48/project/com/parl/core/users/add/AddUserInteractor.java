...
public class AddUserInteractor implements AddUserContract.Interactor {
    ...
    @Override
    public void addUserToDatabase(final Context context, FirebaseUser firebaseUser, String username, String language, boolean loggedIn) {
        DatabaseReference database = FirebaseDatabase.getInstance().getReference();
        User user = new User(firebaseUser.getUid(),
                firebaseUser.getEmail(),
                new SharedPrefUtil(context).getString(Constants.ARG_FIREBASE_TOKEN),username, language, loggedIn);
